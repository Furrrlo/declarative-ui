...
class JDScriptPane {
    ...
    public static DeclarativeComponent<? extends Component> fn(Supplier<Hook.HookScript> script,
...
//                                    (v, newName) -> script.update(s -> s.withName(newName))
                ...
                panelChildren.add(
                        isLafManagerInstalled.get() ?
                                JDPanel.fn(
                                        () -> {
                                            var syntaxTheme = JDScriptPane.syntaxTheme;
                                            if (syntaxTheme == null)
                                                JDScriptPane.syntaxTheme = syntaxTheme = new DarklafRSyntaxTheme();
